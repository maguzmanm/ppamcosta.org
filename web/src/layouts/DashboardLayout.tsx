...
import { Menu, Sun, Moon, LogOut, LayoutDashboard, GitBranch, Building2, Users, MapPin, Calendar, Clock, FileText, Megaphone, BarChart3, Bell, Settings, Accessibility } from 'lucide-react';
...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    if (largeMode) {
      html.classList.add('large-mode');
    } else {
      html.classList.remove('large-mode');
    }
    localStorage.setItem('largeMode', String(largeMode));
  }, [largeMode]);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-t border-border space-y-2">
        <button
          ...
          className={`flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm transition-colors ${largeMode ? 'bg-primary text-white' : 'text-text-secondary hover:bg-surface-hover hover:text-text-primary'}`}
        >
          <Accessibility size={20} />
          {largeMode ? '✓ Letra grande' : 'Letra grande'}
        </button>
    ...
    <div className="flex h-dvh bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden flex items-center gap-3 p-4 border-b border-border bg-surface">
          ...
          <button
            onClick={() => setLargeMode(!largeMode)}
            className={`p-2 rounded-lg transition-colors ${largeMode ? 'bg-primary text-white' : 'hover:bg-surface-hover text-text-secondary'}`}
            title={largeMode ? 'Tamaño normal' : 'Letra grande'}
          >
